...
> Vendored copy pinned to tpope/vim-obsession@5d63e5b, with one local patch:
> `plugin/obsession.vim`'s error catch pattern was `Vim(readfile):E484:`,
> which never matches Neovim's actual exception tag (`Vim(call):E484:`).
> That let E484 leak out of VimEnter the first time `:Obsession` ran in a
> directory with no session file yet. Widened the pattern to just `E484:`.
> Re-check https://github.com/tpope/vim-obsession for an upstream fix
> before updating this copy.
